...
public static class ManModuleEndpoints
{
    public static void MapManModuleEndpoints(this IEndpointRouteBuilder routes)
    {
        ...
        group.MapPost("/", async (CreateManModuleDto manModuleDto, IManModuleRepository repo, IMapper mapper) =>
        {
            var manModule = mapper.Map<ManModule>(manModuleDto);
            await repo.AddAsync(manModule);
            return TypedResults.Created($"/api/manmodule/{manModule.Id}", manModuleDto);
        })
        .WithTags(nameof(ManModule))
